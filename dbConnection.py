...
def conn(head_name,address,lat_,long_,email,mobile):
    db = con.connect(host="localhost", port=3306, user="root", password="$bk97979.", database="db_accident", charset='utf8')
    cur=db.cursor()
...
def view_all_attendance(name):
    ...
    db = con.connect(host="localhost", user="root", password="$bk97979.", database="db_accident", charset='utf8')
    cur = db.cursor()
